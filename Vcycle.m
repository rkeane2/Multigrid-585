function [u,x] = Vcycle(n,levels,v,probtype,mu1,mu2,w,relaxtype,resttype,iters)
...
% note: we always use linear interpolation for prolongation
if isempty(v)
...
%norm(Vgrid(2).f)*sqrt(h) %print out residuals at each grid
...
%norm(Vgrid(i+2).f)*sqrt(h) %print out residuals at each grid
